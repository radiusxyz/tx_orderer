    ...
    fn test_multithreaded_execution_mutex() {
        ...
        for i in 0..thread_count {
            ...
            let handle = thread::spawn(move || {
                for j in 0..1000 {
                    let mut cluster_id_list = cluster_id_list_clone.lock().unwrap();
                    let cluster =
                        cluster_id_list.get_mut(&(Platform::Ethereum, ServiceProvider::Radius));
        ...
        for i in 0..thread_count {
            ...
            let handle = thread::spawn(move || async move {
                for j in 0..1000 {
                    ...
                    let mut cluster_id_list: Value<ClusterIdList> = cluster_id_list_clone
                        .as_ref()
...
mod tests2 {
    ...
    async fn test_send_raw_transaction() {
        ...
        let distributed_key_generation_client = initialize_dkg_client(&config).unwrap();
        ...
        let raw_transaction = RawTransaction::Eth(EthRawTransaction("".to_string()));
    ...
    async fn send_raw_transaction(
        ...
        let start = Instant::now();
        let rollup_metadata = context.get_rollup_metadata(&rollup_id).await.unwrap();
        let duration = start.elapsed();
        tracing::info!("get_rollup_metadata {}: / {:?}", count, duration);
        ...
        let start = Instant::now();
        let mut locked_rollup_metadata = context.get_mut_rollup_metadata(&rollup_id).await.unwrap();
        let duration = start.elapsed();
        tracing::info!("get_mut_rollup_metadata {}: / {:?}", count, duration);
        ...
        .unwrap();
        let duration = start.elapsed();
        tracing::info!("put_with_transaction_hash {}: {:?}", count, duration);
        ...
        tracing::info!("send_raw_transaction - total {}: {:?}", count, duration);
