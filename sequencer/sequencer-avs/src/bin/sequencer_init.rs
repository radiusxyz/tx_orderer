...
async fn main() -> Result<(), Error> {
    ...
    initialize_cluster(&app_state).await?;
...
async fn initialize_cluster(app_state: &AppState) -> Result<(), Error> {
    match app_state
        .ssal_client()
        .is_registered(
            app_state.config().cluster_id(),
        ...
        .await?
    {
        ...
        false => {
            ...
            std::io::stdin().read_line(&mut rollup_address).unwrap();
            let rollup_address = rollup_address.trim();

            app_state
                .ssal_client()
                .initialize_cluster(rollup_address, app_state.config().sequencer_rpc_url())
                .await?;
            tracing::info!("Successfully initialized the cluster.");
        }
    }
